...
x[ind] = x[ind] + rnorm(sum(ind), 5, 1/12)

y = sample(heart)
ind = rbinom(length(y), 1, 0.5) == 1
y[ind] = y[ind] + rnorm(sum(ind), 2, 1/3)
...
Uniform Distribution:
```{r}
ggplot(x,aes(x=target, y=age, fill=target)) +
geom_boxplot() +
facet_wrap(~ sex)+
scale_fill_manual(values=c("#97BE11","#DC1E0B"))

ggplot(y,aes(x=target, y=age, fill=target)) +
geom_boxplot() +
facet_wrap(~ sex)+
scale_fill_manual(values=c("#97BE11","#DC1E0B"))
...
```{r}
x2 = sample(heart)
ind = rbinom(length(x2), 1, 0.5) == 1
x2[ind] = x2[ind] + rnorm(sum(ind), 0, 1)

y2 = sample(heart)
ind = rbinom(length(y2), 1, 0.5) == 1
y2[ind] = y2[ind] + rnorm(sum(ind), 0, 1.23)
